...
void main()
{
	...
	vec3 lightPos = vec3(0.0, 0.0, 0.0); // Position of the light source
	...
	vec3 diffuse = diffStrength * vec3(1.0, 1.0, 1.0); // White light
    
    // Specular
    vec3 viewDir = normalize(-fragPos); // assuming the camera is at the origin
	vec3 reflectDir = reflect(-lightdir, normal);
	float specStrength = pow(max(dot(viewDir, reflectDir), 0.0), phongExp);
	vec3 specular = specStrength * vec3(1.0, 1.0, 1.0); // White light
